...
MODEL_PATH = os.path.join(BASE_DIR, 'models', 'Arrohan_Custom_Best.pt')#custom_best.pt
...
# Camera settings
# Replace this with your phone's IP camera URL (e.g., http://192.168.1.x:8080/video)
# You can often get this from apps like "IP Webcam" on Android
CAMERA_URL = "http://192.168.1.28:8081/video" 
...
def detection_loop():
    ...
    # Open webcam (Phone Camera)
    print(f"Attempting to connect to phone camera at: {CAMERA_URL}")
    cap = cv2.VideoCapture(CAMERA_URL)
    
    if not cap.isOpened():
        print(f"Error: Could not open phone camera at {CAMERA_URL}")
        print("Please check if the IP Webcam app is running and the URL is correct.")
        return
    
    print(f"Phone camera opened. Streaming at {TARGET_FPS} FPS")
    ...
    try:
        while not stop_event.is_set():
            ...
            if not ret:
                ...
                # Try to reconnect or break? Breaking for now to avoid infinite error loops
                break
...
if __name__ == '__main__':
    print("=" * 60)
    print("YOLO Real-Time Detection Server (Phone Camera Mode)")
    print("=" * 60)
